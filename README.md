...
* **Object Storage:** **ImageKit.io** (Production) / MinIO (Local Development)
...
### Storage (MinIO)
* `MINIO_ROOT_USER` (Root admin user)
* `MINIO_ROOT_PASSWORD` (Root admin password)
* `MINIO_ACCESS_KEY` (Dedicated backend key)
* `MINIO_SECRET_KEY` (Dedicated backend secret)
* `MINIO_INTERNAL_URL` (e.g., `http://minio:9000`)
* `MINIO_BUCKET_NAME`

### External Services & CI/CD
* `NGROK_AUTHTOKEN` (Required for local tunneling)
* `RUNNER_PAT_TOKEN` (GitHub Personal Access Token for self-hosted runners)
...
1. Create a directory for the project (e.g., `gamevault`).
2. Clone the backend repository: `git clone https://github.com/Kirtasth/GameVault-Backend.git`
3. Clone the frontend repository: `git clone https://github.com/Kirtasth/GameVault-Frontend.git`
4. Create a `compose.yaml` file in the project root (the directory created in step 1) and copy the content from the **Compose** section at the bottom of this README.
5. Build the Docker images:
   ```bash
   docker build -t gamevault-backend:latest ./GameVault-Backend
   docker build -t gamevault-frontend:latest ./GameVault-Frontend
   ```
6. Start the infrastructure: `docker compose --profile status up -d`
7. Access the API at `http://localhost:8080`
8. View the Health Check at `http://localhost:8080/actuator/health`
9. Access the MinIO Console at `http://localhost:9001`
10. Connect to the Database at `localhost:5432`
...
The application is deployed to **Fly.io** and follows a modern cloud-native infrastructure.

* **Live API:** [https://gamevault-backend.fly.dev](https://gamevault-backend.fly.dev)
* **Health Check:** [https://gamevault-backend.fly.dev/actuator/health](https://gamevault-backend.fly.dev/actuator/health)

### Infrastructure
* **Backend:** Hosted on Fly.io (Machine: `shared-cpu-2x`, `1GB RAM`)
* **Database:** PostgreSQL instance on **Supabase**
* **Images:** Managed via **ImageKit.io**
...
To run the test suite, execute `./mvnw test` in your terminal.
...
We use self-hosted runners (`backend-runner`) deployed via Docker Compose to execute our workflows. The runner mounts the host's `/var/run/docker.sock`, allowing it to spin up Testcontainers and build images natively.

### Production Pipeline
When code is pushed to the `pro` branch, a dedicated workflow:
1.  Runs the full test suite.
2.  Builds a Docker image using the `pro` Spring profile.
3.  Pushes the image to the Fly.io registry.
4.  Deploys the new version to Fly.io automatically.

> **Note:** Production secrets (Supabase credentials, ImageKit keys, Fly API tokens) are securely managed via GitHub Secrets and Fly.io Secrets.
